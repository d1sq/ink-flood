...
<script lang="ts">
  ...
  import { getEvent, EVENTS, formatTime } from '../data/events';
  ...
  const SLOT_LABELS = { matter: 'Материя', word: 'Слово', vision: 'Зрение' };
...
<div class="ink-flood-dashboard" bind:this={elementRoot}>
  <!-- Header -->
  ...
  <main class="dashboard-content">
      ...
      <section class="tab-overview">
        ...
        <div class="time-controls">
          ...
          <div class="time-buttons">
            <button on:click={() => handleAdvance(10)}>+10 мин</button>
            <button on:click={() => handleAdvance(15)}>+15 мин</button>
            <button on:click={() => handleAdvance(30)}>+30 мин</button>
        ...
        <!-- Glitch Notice -->
        {#if $activeGlitch}
          <div class="glitch-notice">
            DC {$activeGlitch.dcModifier > 0 ? '+' : ''}{$activeGlitch.dcModifier}
          </div>
        {/if}
    ...
    {:else if activeTab === 'events'}
      <section class="tab-events">
        ...
        {#each availableEvents as ev}
          ...
          <div class="event-row {getStatusClass(ev.status)}">
            ...
            {#if ev.status === 'available'}
              <button class="btn-complete" on:click={() => handleCompleteEvent(ev.id)}>
                Выполнено
              </button>
            {/if}
          </div>
    ...
    {:else if activeTab === 'glitches'}
      <section class="tab-glitches">
        <h3>Глитчи</h3>
        {#if $activeGlitch}
          <p><strong>DC модификатор:</strong> +{$activeGlitch.dcModifier}</p>
        ...
        {:else}
          <p>Нет активных глитчей (циклы 1–6).</p>
        {/if}
      </section>
    {/if}
  </main>
